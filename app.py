...
try:
    ...
    print(f"Warning: Could not import solver: {e}")
    print("Will run in mock mode for testing")
...
class MockDun163:
    ...
    def run(self):
        ...
        self.resp_json2 = {
            'result': True,
            'validate': 'mock_validate_123456789'
        }
        return True

if SOLVER_AVAILABLE:
    SolverClass = Dun163
    logger.success(" Using real Yidun solver")
else:
    SolverClass = MockDun163
    logger.warning(" Using mock solver (for testing only)")
...
class YidunAPIServer:
    ...
    async def _startup(self) -> None:
        ...
        logger.info("=" * 60)
        logger.info(" YIDUN CAPTCHA SOLVER API")
        logger.info("=" * 60)
        ...
        if not self.mock_mode and SOLVER_AVAILABLE:
        ...
        if not self.mock_mode:
            ...
            logger.warning(" Running in MOCK MODE - no real captcha solving")
            ...
            <div class="container mx-auto px-4 py-8">
                <div class="max-w-4xl mx-auto">
                    <div class="bg-gray-800 rounded-lg shadow-xl p-6 mb-8">
                        <h1 class="text-3xl font-bold text-center mb-2"> Yidun Captcha Solver API</h1>
                        <p class="text-gray-400 text-center mb-6">NetEase Yidun (dun.163.com) solving service</p>
                        
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                            <div class="bg-gray-700 p-3 rounded-lg text-center">
                                <div class="text-green-400 text-xl mb-1">{'✅' if self.model else '⏳'}</div>
                        ...
                        <div class="grid md:grid-cols-2 gap-6">
                            <div class="bg-gray-700 p-4 rounded-lg">
                                ...
                                <ul class="space-y-1 text-sm">
                                    ...
                                    <li><span class="text-yellow-400">fp_h</span> - Fingerprint host (optional)</li>
                                    <li><span class="text-yellow-400">ua</span> - User-Agent (optional)</li>
                                    <li><span class="text-yellow-400">domain</span> - API domain (optional)</li>
                                </ul>
                            </div>
...
# GET request
curl "http://{self.host}:{self.port}/solve?id=fef5c67c39074e9d845f4bf579cc07af"

# POST request
curl -X POST "http://{self.host}:{self.port}/solve" \\
...
curl -X POST "http://{self.host}:{self.port}/solve" \\
    ...
    async def health(self):
        ...
        return jsonify({
            ...
            'memory_percent': round((memory/1024)*100, 1),
            'model_loaded': self.model is not None if not self.mock_mode else True,
            'js_loaded': self.js_ctx is not None if not self.mock_mode else True,
    ...
    async def stats(self):
        ...
        return jsonify({
            ...
            'model': {
                'loaded': self.model is not None if not self.mock_mode else True,
                'mock_mode': self.mock_mode
            },
            'config': {
    ...
    async def memory_stats(self):
        ...
        return jsonify({
            'memory': {
                ...
                'peak_mb': round(memory, 2)  # Simplified
            },
            'garbage_collector': {
                'collected': collected,
                'counts': gc.get_count(),
    ...
    async def solve_endpoint(self):
        ...
        if self.debug:
            ...
            logger.debug(f"[{req_id}]   Domain: {domain}")
        ...
        try:
            ...
            if self.mock_mode:
                logger.info(f"[{req_id}]  Using mock solver")
                solver = MockDun163(
                    id_=id_,
                    referer=referer,
                    fp_h=fp_h,
                ...
                if not self.model or not self.js_ctx:
                    logger.warning(f"[{req_id}] Model not ready")
                    return jsonify({
                        'success': False,
                        'error': 'Model still loading, please wait a few seconds'
                    }), 503
            ...
            def run_solver():
                try:
                    ...
                    logger.error(f"[{req_id}] Solver error: {e}")
                    return False
            
            success = await loop.run_in_executor(None, run_solver)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Yidun Captcha Solver API ')
    ...
    parser.add_argument('--mock', action='store_true',
                       help='Run in mock mode (no real solving)')
...
def main():
    ...
    if 'RAILWAY_ENVIRONMENT' in os.environ or 'RAILWAY_STATIC_URL' in os.environ:
        args.host = '0.0.0.0'
        logger.info(" Detected Railway.app environment")
